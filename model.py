...
class PairwiseRankingLoss(torch.nn.Module):
    ...
    def forward(self, im, s):
        ...
        cost_s = torch.max(Variable(torch.zeros(scores.size()[0], scores.size()[1])), (margin-diagonal).expand_as(scores)+scores)
        # compare every diagonal score to scores in its row (i.e, all contrastive sentences for each image)
        cost_im = torch.max(Variable(torch.zeros(scores.size()[0], scores.size()[1])), (margin-diagonal).expand_as(scores).transpose(1, 0)+scores)
